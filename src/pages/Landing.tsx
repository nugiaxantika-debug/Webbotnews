  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-200 font-sans">
      ...
      <nav className="fixed top-0 w-full z-40 bg-neutral-950/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold text-white tracking-tight">{webConfig.title}<span className="text-emerald-400">{webConfig.highlight}</span></span>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-40 pb-20 px-6 max-w-7xl mx-auto text-center">
        ...
        <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tight mb-8 leading-tight max-w-4xl mx-auto">
          {webConfig.heroTitle}
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <button 
            ...
            className="bg-emerald-500 hover:bg-emerald-600 text-white px-8 py-4 rounded-full font-bold text-lg transition-colors flex items-center gap-2 w-full sm:w-auto justify-center"
          >
      ...
      {/* Features */}
      <section className="py-24 bg-neutral-900/50 border-y border-white/5 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-3xl font-bold text-center text-white mb-16">Fitur Unggulan</h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-neutral-950 border border-neutral-800 p-8 rounded-3xl hover:border-emerald-500/30 transition-colors">
              <div className="bg-blue-500/10 w-14 h-14 rounded-2xl flex items-center justify-center mb-6">
                <MessageCircle className="w-7 h-7 text-blue-400" />
              </div>
              <h3 className="text-xl font-bold text-white mb-3">{webConfig.feature1Title}</h3>
              <p className="text-neutral-400 leading-relaxed">{webConfig.feature1Desc}</p>
            </div>
            <div className="bg-neutral-950 border border-neutral-800 p-8 rounded-3xl hover:border-emerald-500/30 transition-colors">
              ...
              <h3 className="text-xl font-bold text-white mb-3">{webConfig.feature2Title}</h3>
              <p className="text-neutral-400 leading-relaxed">{webConfig.feature2Desc}</p>
            </div>
            <div className="bg-neutral-950 border border-neutral-800 p-8 rounded-3xl hover:border-emerald-500/30 transition-colors">
              <div className="bg-amber-500/10 w-14 h-14 rounded-2xl flex items-center justify-center mb-6">
                <Shield className="w-7 h-7 text-amber-400" />
              </div>
              <h3 className="text-xl font-bold text-white mb-3">{webConfig.feature3Title}</h3>
              <p className="text-neutral-400 leading-relaxed">{webConfig.feature3Desc}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 text-center">
        <h2 className="text-3xl font-bold text-white mb-6">{webConfig.pricingTitle}</h2>
        <p className="text-neutral-400 mb-12 max-w-xl mx-auto">{webConfig.pricingDesc}</p>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto text-left">
          ...
          <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl">
            <div className="flex justify-between items-end mb-8">
              <div>
                <h3 className="text-2xl font-bold text-white">{webConfig.plan1Name}</h3>
                <p className="text-neutral-400 mt-1">{webConfig.plan1Duration}</p>
              </div>
          ...
          <div className="bg-gradient-to-b from-neutral-900 to-neutral-950 border border-amber-500/30 p-8 rounded-3xl relative overflow-hidden">
            ...
            <div className="flex justify-between items-end mb-8">
              <div>
                <h3 className="text-2xl font-bold text-white flex items-center gap-2">{webConfig.plan2Name}</h3>
                <p className="text-neutral-400 mt-1">{webConfig.plan2Duration}</p>
              </div>
              <div className="text-right">
      ...
      {/* Footer */}
      <footer className="border-t border-neutral-800 bg-neutral-950/50 py-12 mt-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col items-center md:items-start gap-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold text-white tracking-tight">{webConfig.title}<span className="text-emerald-400">{webConfig.highlight}</span></span>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 text-sm text-neutral-400">
            <a href={`mailto:${webConfig.contactEmail}`} className="hover:text-white transition-colors flex items-center gap-2">
              <Mail className="w-4 h-4" /> {webConfig.contactEmail}
            </a>
            <a href={`https://wa.me/${webConfig.contactPhone.replace(/\D/g, '')}`} className="hover:text-white transition-colors flex items-center gap-2">
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 relative">
            <button 
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <button 
                type="submit"
                className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 rounded-xl transition-colors mt-2"
              >
